...
def do_clustering(args, high_dim_vectors):
    ...
    if clustering_algorithm == "kmeans":
        clustering = KMeans(n_clusters=args.kmeans_num_clusters, random_state=0).fit(
            high_dim_vectors
        )
...
def sample_middle_confidence_data(cluster_labels, confidences, n, low_th=25, up_th=75):
    ...
    for i in range(num_clusters):
        ...
        middle_indices = sorted_indices[
            (cluster_confidences >= lower_threshold)
            & (cluster_confidences <= upper_threshold)
        ]
...
def main():
    ...
    middle_confidence_samples = sample_middle_confidence_data(
        cluster_labels, ppl_array, args.sample_num, args.low_th, args.up_th
    )
